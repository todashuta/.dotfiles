...
- 個人的には ~%USERPROFILE%~ 以下に ~.emacs.d~ を設置したい。
- OSの設定で ~HOME~ 環境変数を設定すればいいんだけど、他のツールに影響を与え得るのでなるべくEmacsだけに閉じた設定方法にしたい。
- ++ というわけで ~%APPDATA%\.emacs.d~ に下記のファイルを置いて ~%USERPROFILE%\.emacs.d~ に置いた各種設定が使われるようにしている。 ++
- Emacs用のレジストリで ~HOME~ を設定することができる。こっちのほうが良さそう。
...
#+begin_src emacs-lisp :tangle no
  ...
  (load (expand-file-name (concat user-emacs-directory "init.el")) t)
#+end_src
** ++ %APPDATA%\.emacs.d\early-init.el ++
#+begin_src emacs-lisp :tangle no
  (setenv "HOME" (getenv "USERPROFILE"))
  (load (expand-file-name (concat user-emacs-directory "early-init.el")) t)
#+end_src
* ヘッダ
