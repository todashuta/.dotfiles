...
#+end_src
** M-x list-packages でgpgのエラー（Failed to verify signature archive-contents.sig）が発生する場合の対処
- PATHの状況でmsysのgpgが使われると発生する(?)
- 参考
  - https://www.reddit.com/r/emacs/comments/ymzw78/comment/l77lqkt/
*** エラーが発生する環境では init-local.el に下記を追記する
...
#+begin_src emacs-lisp
  (leaf ddskk
    ...
    :config
    (let* ((jisyo-dir (file-name-as-directory
                       ;; skk-get-jisyo-directory
                       (concat user-emacs-directory "skk-dict")))
